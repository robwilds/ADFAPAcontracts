  ...
  animations: [
    trigger("inOutAnimation", [
      transition(":enter", [
        ...
        animate("3s ease-out", style({ height: 300, opacity: 1 })),
      ]),
      transition(":leave", [
...
export class DocumentsComponent implements OnInit {
    ...
    this.sub = this.route.params.subscribe(
      (value) =>
        (this.currentFolderId =
          value["fid"] == null
            ? "5be4a4cc-f413-4f28-8329-dce29671b224"
            : value["fid"])
    );
  }
  ...
  nodeClicked(event: NodeEntityEvent) {
    if (this.documentList.selection[0].entry.isFile === true) {
      ...
      console.log(
        ...
        this.node.properties["ContractManagement:associations"]
      );
    }
  }
  ...
  folderAssociationsCustomAction(event) {
    ...
    console.log(
      "association clicked",
      this.folderNodeAssociation,
      "show state ",
      this.showAssocations
    );
